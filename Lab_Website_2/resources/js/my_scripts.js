...
function LoadStatsPage(){
	...
	for(var rowCount = 2; rowCount < table.rows.length; rowNum++)
	{
		var buffScore = table.cells[2].innerHTML;
		var oppScore = table.cells[3].innerHTML;
		...
		else if(buffScore < oppScore)
		{
			row.cells[4].innerHTML += row.cells[1].innerHTML;
			l++;
		}

		var newWs = document.getElementById("wins");
		newWs.innerHTML += w;

		var newLs = document.getElementById("losses");
		newLs.innerHTML += l;
	}
}
...
function loadPlayersPage(){
	var pMenu = document.getElementById("player_selector");
	var none = "";

	for(var i = 0; i < players.length; i++)
	{
		var playerName = players[i].name;
		none = none + "<a class = 'dropdown-item' href='#' onclick='switchPlayers("+ i +")'>" + playerName + "</a>";
	}

	pMenu.innerHTML=none;
}
